...
\tikzset{%
  % Specifications for style of nodes:
    ...
    stateTransition/.style = {-stealth, thick},
}
...
\begin{tikzpicture}
	...
	\node[inputNode] (x3) at (-2, -0.0) {$\tiny x_3$};
	\node[inputNode] (xn) at (-2, -1.5) {$\tiny x_n$};

	\draw[stateTransition] (x1) to[out=0,in=120] node [midway, sloped, above] {$w_1$} (x);
	\draw[stateTransition] (x2) to[out=0,in=150] node [midway, sloped, above] {$w_2$} (x);
	\draw[stateTransition] (x3) to[out=0,in=180] node [midway, sloped, above] {$w_3$} (x);
	\draw[stateTransition] (xn) to[out=0,in=240] node [midway, sloped, above] {$w_n$} (x);
	\draw[stateTransition] (x) -- (4,0) node [midway,above] {$\sigma\left(b + \sum\limits_{i=1}^{n}{x_iw_i}\right)$};
